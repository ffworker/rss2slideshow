...
class CustomerBrandTests(unittest.TestCase):
    ...
    def test_shipped_bistro_demo_uses_primary_logo_and_soft_colors(self):
        ...
        with main.app.test_client() as client:
            ...
            self.assertIn("grid-template-columns:repeat(5,minmax(0,1fr))", player)
            self.assertIn("font-size:clamp(18px,1.5vw,30px)", player)
            self.assertIn("font-size:clamp(27px,2.35vw,47px)", player)
            self.assertIn("border-left:2px solid var(--accent)", player)
